...
import { level_ids, timeseries_ids, BASIN_COLORS, poolDisplay } from "../config.js"
...
export const pie = (
  ...
  if (percentFull > 1) {
    ...
    console.warn("percentFull must be between 0 and 1")
  } 
  ...
  svg
    ...
    .on("mouseover", function (e) {
      ...
      d3.select(this).attr("transform", `translate(${cx + offsetX}, ${cy + offsetY})`)
    })
  ...
  const text = textGroup
    ...
    .text(missing_data ? title + " Missing ⚠" : title + " " + (Math.round(percentFull * 100) + "%"))
...
export function createBasinPie(
  svg,
  basinProjects,
  { radius, width, height, viewBoxWidth, viewBoxHeight, basin, pool, levelData, tsData, labels, cx, cy, fontSize, STROKE_WIDTH = 1 }
) {
  ...
  locations.forEach((location) => {
    ...
    if (!TOF) {
        ...
        alert("Top of Conservation is not defined for: " + location + level_ids.TOC)
    ...
    let stor_filled =
      tsData[
        pool == "flood_pool" ? location + timeseries_ids.FCP : location + timeseries_ids.Cons
      ]?.[1]
  ...
  if (labels.length > 0) {
    labels.forEach((label) => {
      ...
      label.text &&
        _text.text(
          label.text
            .replace("%total_stor", Number(parseInt(total_stor).toFixed(0)).toLocaleString())
            .replace("%percent_full", Math.round((total_stor / total_incr) * 100))
            .replace("%pool", poolDisplay[pool])
            .replace("%total_incr", Number(parseInt(total_incr).toFixed(0)).toLocaleString())
        )
    })
  }
  return svg
