...
export default function BasinPie({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const config = { level_ids, timeseries_ids, BASIN_COLORS, poolDisplay };
      createBasinPie(svg, basinProjects, options, config);
  ...
  if (isPending)
    return (
      <Skeleton
        ...
        className="m-auto"
      >
        <div className="bg-slate-50 text-black font-semibold text-center relative left-1/2 top-1/2 rounded-xl -translate-x-1/2 -translate-y-1/2 text-2xl w-1/5">
          {progress}%
        </div>
      </Skeleton>
    );
